  ...
  _.each(config, function(value, key) {
    ...
    if (key === "use_env_variable") {
      if (process.env[value]) {
		var u = require('url').parse(process.env[value]);

		var dialect = u.protocol.split(':')[0];
		var auth = {
			username: null,
			password: null
		};
		var database = u.path.replace('/','');
		if(u.auth) {
			auth.username = u.auth.split(':')[0];
			auth.password = u.auth.split(':')[1];
		}
